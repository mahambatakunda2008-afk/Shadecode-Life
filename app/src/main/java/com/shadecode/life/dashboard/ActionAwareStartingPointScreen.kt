    ...
    Column(
        ...
        decision?.let { decisionValue ->
            NextCapabilityCard(
                ...
                onStart = { onStartAction(DevelopmentDecisionEngine.actionFor(decisionValue)) }
            )
        }
        ...
        Button(onClick = onViewDevelopmentMap, modifier = Modifier.fillMaxWidth()) { Text("Open development map") }
...
@Composable
private fun NextCapabilityCard(
    decision: DevelopmentDecisionEngine.Decision,
    onStart: () -> Unit
) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Button(onClick = onStart, modifier = Modifier.fillMaxWidth()) { Text("Develop this") }
        }
    }
}
