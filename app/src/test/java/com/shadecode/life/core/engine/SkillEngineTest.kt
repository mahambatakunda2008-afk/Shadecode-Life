...
class SkillEngineTest {
    ...
    fun distinctEvidenceDaysRespectTheSuppliedTimezone() {
        val evidence = listOf(
            Evidence("boundary-1", DevelopmentDomain.BODY, "Night measurement", "body_capacity", 10.0, "reps", kind = EvidenceKind.MEASUREMENT, recordedAt = Instant.parse("2026-09-10T21:30:00Z")),
            Evidence("boundary-2", DevelopmentDomain.BODY, "Early measurement", "body_capacity", 11.0, "reps", kind = EvidenceKind.MEASUREMENT, recordedAt = Instant.parse("2026-09-10T22:30:00Z"))
        )
        val progress = SkillEngine.progress(evidence, ZoneId.of("Africa/Harare"))
